...
kubectl create secret generic cosmos-db-secret \
  ...
  --from-literal=ConnectionString="$(az cosmosdb keys list \
    --name $(az deployment group show -g rg-azure-sre-agent-demo-lab -n main-aks --query properties.outputs.cosmoS_ACCOUNT_NAME.value -o tsv) \
    --resource-group rg-azure-sre-agent-demo-lab \
...
if [ -z "$AGC_FQDN" ]; then
  AGC_FQDN=$(az deployment group show -g rg-azure-sre-agent-demo-lab -n main-aks \
    --query properties.outputs.agC_FRONTEND_FQDN.value -o tsv)
fi
