...
このガイドでは、AzureSREAgentDemoLab アプリケーションに Azure SRE Agent を統合し、Azure Monitor をインシデント ソースとして、Azure relay 経由で Azure DevOps、GitHub、またはその両方へインシデントを連携する構成を設定します。

## 前提条件

- Azure サブスクリプション (所有者または共同作成者ロール)
- Azure CLI (`az`) インストール済み
- GitHub アカウント (AzureSREAgentDemoLab リポジトリへのアクセス)
...
- HTTP トリガーの **完全な callback URL**
- レイテンシしきい値と失敗リクエスト数しきい値

ここで使うのは Logic App の概要画面に出る URL ではなく、**HTTP トリガーの callback URL** です。`incidentRelayCallbackUrl` に渡す値は、次のように trigger のパスと署名を含む完全な invoke URL である必要があります。

```text
https://.../workflows/.../triggers/When_an_HTTP_request_is_received/paths/invoke?...&sig=...
```

`/triggers/` や `sig=` を含まない URL を渡すと、Action Group では Logic App が設定されているように見えても、Azure Monitor からワークフローは呼び出されません。
...
SRE Agent は GitHub コネクタを使ってリポジトリ コンテキストを読み取り、GitHub ルートを選ぶ場合は Azure relay が作成した Issue の参照や更新にも利用します。
...
Azure relay が作成した Azure DevOps の作業項目を SRE Agent が読む、コメントする、履歴を見る場合は Azure DevOps コネクタも登録します。
